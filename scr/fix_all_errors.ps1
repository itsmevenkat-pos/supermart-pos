# fix_all_errors.ps1 – Fixes all remaining errors
$base = "lib"
                  ...
                  child: Column(
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          _actionButton('Customer', Icons.person, primaryColor, 'F1', _showCustomerDialog),
                          _actionButton('Discount', Icons.percent, primaryColor, 'F2', _showDiscountDialog),
                          _actionButton('Hold', Icons.save, primaryColor, 'F3', () {
                            ref.read(cartProvider.notifier).clearCart();
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Bill held!'), backgroundColor: Colors.blue),
                            );
                          }),
                          _actionButton('Cash', Icons.money, primaryColor, 'F4', () {
                            _showPaymentDialog({'cash': grandTotal});
                          }),
                          _actionButton('UPI', Icons.qr_code, primaryColor, 'F5', () {
                            _showPaymentDialog({'upi': grandTotal});
                          }),
                          _actionButton('Card', Icons.credit_card, primaryColor, 'F6', () {
                            _showPaymentDialog({'card': grandTotal});
                          }),
                          _actionButton('Credit', Icons.account_balance, primaryColor, 'F7', () {
                            if (notifier.customer == null) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                          ...
                          _actionButton('Share', Icons.share, primaryColor, 'F8', () async {
                          ...
                          _actionButton('Quotation', Icons.description, primaryColor, 'F9', _showQuotationDialog),
...
class _PurchaseFormScreenState extends ConsumerState<PurchaseFormScreen> {
  ...
  void _addProduct(Product product) {
    ...
    _items.add(PurchaseItem.create(
      ...
      salesPrice: product.retailPrice, // ✅ Fixed: retailPrice instead of sellingPrice
...
'@
    ...
    # ---------- FIXED PURCHASE ITEM MODEL (fix barcode & freeQuantity) ----------
...
Write-Host "`n✅ All errors fixed!" -ForegroundColor Cyan
